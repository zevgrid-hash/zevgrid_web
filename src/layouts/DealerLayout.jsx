...
//       } catch (error) {
//         if (!ignore) {
//           setDealerError(error.message || "Dealer details could not be loaded.");
        ...
        <div className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            <div className="hidden text-right sm:block">
              {dealer ? (
                <>
                  <p style={s.dealerName}>{dealer.dealership}</p>
                  <p style={s.dealerStatus}>{dealer.status}</p>
                </>
              ) : dealerError ? (
                <p style={{ ...s.dealerStatus, color: "#ef4444" }}>Unavailable</p>
              ) : (
                // Loading skeleton
                <>
                  <div
